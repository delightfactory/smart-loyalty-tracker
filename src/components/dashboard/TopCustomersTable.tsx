          ...
          <table className="w-full text-right">
            ...
            <tbody>
              {topCustomers.map((c, idx) => (
                <tr key={c.id} className={idx % 2 === 0 ? 'bg-blue-50/50' : 'bg-white'}>
                  <td className="p-2 font-bold text-blue-700">{formatNumberEn(idx + 1)}</td>
                  <td className="p-2 font-semibold">{c.name}</td>
                  <td className="p-2">{formatNumberEn(c.purchases)}</td>
                  <td className="p-2 text-green-700 font-bold">{formatAmountEn(c.totalAmount)}</td>
                </tr>
              ))}
            </tbody>
          </table>
