...
export function dbCustomerToAppCustomer(dbCustomer: any): Customer {
  return {
    ...
    currentPoints: Number(dbCustomer.current_points ?? 0),
    pointsEarned: Number(dbCustomer.points_earned ?? 0),
    pointsRedeemed: Number(dbCustomer.points_redeemed ?? 0),
    classification: Number(dbCustomer.classification ?? 0),
    level: Number(dbCustomer.level ?? 0),
    creditBalance: Number(dbCustomer.credit_balance ?? 0),
    openingBalance: dbCustomer.opening_balance ?? 0,
    governorate: dbCustomer.governorate ?? '',
    city: dbCustomer.city ?? '',
    earnPointsEnabled: dbCustomer.earn_points_enabled ?? true,
...
export function appCustomerToDbCustomer(customer: Customer | Omit<Customer, 'id'>): any {
  return {
    ...
    earn_points_enabled: customer.earnPointsEnabled,
